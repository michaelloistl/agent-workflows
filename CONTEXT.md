...
**Tracer-bullet**:
A thin, independently-buildable vertical slice of a spec — a standalone issue carrying a textual `## Parent` reference to the spec and a `## Blocked by` section. The `implement` issue-shape guard refuses an epic — an issue with sub-issues of its own — outright, and refuses a native GitHub sub-issue whose native parent *disagrees* with its `## Parent`; one that AGREES is a tracer-bullet and is built, which is how a tracker sync (a Linear mirror of the parent/child edge) is meant to look. The native link never replaces the textual one: the orchestrator resolves a spec's membership through `## Parent` alone, so a slice linked only by the hierarchy is invisible to a spec run. Where a native parent is present the *status view* reads it in preference to the body (see *spec tree*), while the orchestrator still goes by the text. The *dependency* edge is read from both sources at once: a slice's blockers are its native `blockedBy` edges **unioned** with its `## Blocked by` refs, everywhere the fleet reads dependencies — the status view, the orchestrator, and the `implement` blocked-by *guard*. The graph and the guard read one rule but answer different questions: the graph orders a spec's slices among themselves and ignores a blocker that is not one of them, while the guard gates a single run on *any* open blocker, which is why a slice named directly by a human still stops at the right time.
...
**Final PR**:
The one PR a spec produces for a human: `spec branch → default`, opened as a **draft** by *advance* once the last tracer-bullet closes, carrying `Closes #<spec>` so merging it closes the spec. Identified by its **head and base branches** — an open PR in the repo itself from the spec branch to the base advance opened it against (`finalPrBase`, the one function both the opening and the reading go through), which is the one pair GitHub allows only one open PR for per head repository, so a fork's branch of the same name is never it — and never by a label: the `agent:review-pr` a consuming repo may put on it is a *trigger label*, retired by the review run the moment it starts and switchable off entirely (`finalPrReview: false`), so it is present only in the window between opening and reviewing. The *status view* shows it as a row of its own once it exists, which is what tells "advance has not opened it yet" apart from "it is open and waiting on you".
_Avoid_: preview PR, spec PR, integration PR
...
**Quota headroom**:
The share of the account's rolling session and weekly limits still unconsumed — what the status view leads with, above the tree. Account-global rather than per-spec, so it is a line and never a column. It is the fleet's bound too, not just the operator's: every verb runs under a subscription token, so CI drains the same windows interactive work does — and drains them invisibly, since consuming a week's headroom moves no label and closes no issue. Distinct from the **run ceiling**, which bounds one run; headroom bounds all work everywhere. The rendered line states each window in the *complement* — `week 39% used` — because that is the direction the source reports and the direction the colours ramp; the word `used` is therefore never dropped from it, since a bare `39%` under a line labelled quota reads either way round.
_Avoid_: usage (as the name for this — consumption already incurred points at the opposite decision from the one headroom is read to make), cost, spend, budget, quota (bare — the limit, not what is left of it)

**Running package version**:
The version declared by the exact package copy executing a command — read from that copy's own manifest, never from the consuming repo's dependency range, a git ref, a lockfile comparison or a remote release. What the *status view*'s footer states, so a pane, a screenshot and a redirected file all say which release produced them. Resolved once when the process starts and held for its life, so a `--watch` open across an install never claims a release it is not running; unavailable for any reason it renders as `agent-workflows version unknown`, since optional metadata cannot suppress the view it decorates.
_Avoid_: installed version, declared version, latest version (each names a different thing the footer is not)
...
**Local-run marker**:
`agent:local` on a *spec* issue: an attended local run owns that spec's sequencing, so unattended **advance** stands down while it is present. Neither a trigger label (nothing triggers on it) nor a state label (it is scoped to a run, not to an issue's progress) — it exists because advance is fired by a *merge* rather than a label, and a local run merges too. Claimed before the first merge and released when the run **completes**; a run that **halts** keeps it, because the spec is still the developer's and the merge that would make CI build the stopped slice may still be in flight (ADR-0009). Nothing is stranded by that: the loop prints what hands the spec back to CI — remove the label *and* re-run the advance run that stood down, since removing it lifts the stand-down but dispatches nothing on its own — and a marker found while holding the local *lock* was left by a run that is not alive (a halt, or a crash) and is *reclaimed*, never refused. A *dry run* never merges, so it never takes one.
