...
- A halted attended run leaves `agent:local` on the spec issue, so that spec is out of CI's hands until a human acts. That is the point, and it is the cost: a developer who abandons a local run and wants CI to carry on must remove the label *and* restart CI on that spec. The second step is not an oversight in the design — advance is fired by a *merge*, and the run that this spec's last merge fired has already read the marker, stood down and exited, so nothing re-reads the label later. Removing it lifts the stand-down for the *next* merge; re-running the stood-down advance run (or labelling the next tracer-bullet `agent:implement`) is what gets CI moving now. The loop's exit line names both, and the next attended run reclaims the marker if the developer resumes locally instead.
...
- **Reclaim becomes the ordinary resume path**, not a crash recovery, so the sentence the resuming run prints reports a handover rather than damage — it no longer calls the marker *stale* or the previous run one that *did not release it*, both of which are now the normal case.
- **The deadlocked exit is a halt.** A run that finds no ready slice on an incomplete spec (a dependency cycle in the remainder) stops short and waits for a human, exactly like every other halt, so it is recorded as one: it keeps the marker, the summary says halted, and the run log carries the transition. Before this ADR the distinction did not show, because every exit released the marker anyway.
