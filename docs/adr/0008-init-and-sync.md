...
**Nothing is written until the whole plan is accepted**, in the shape the spec loop already uses: print the blast radius, a non-interactive stdin declines, `--yes` pre-accepts, `--dry-run` prints and stops. Three refusals are deliberate and permanent:
