...
const context: RunContext = {
  ...
  // How an attended run finalizes (issues #57, #143). Unset (unattended, or an attended
  // `auto` run) keeps the finalize tail — full parity; `ask`/`never` drop it, along with
  // the in-progress status write, for the verbs that read it (`implement`, `review-pr`).
  // `FINALIZE_TAIL_ONLY` selects the tail-only slice the attended `ask` path runs after
  // its confirmation.
