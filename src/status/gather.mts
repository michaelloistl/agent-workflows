...
// still only textual. The scan is what is left for a repo with no native hierarchy.
...
export function gatherIssues(
  ...
  // The full-repo scan is the last resort, for a spec with no native hierarchy at all:
  // the timeline read is newer and less proven, so an unmigrated repo keeps the path it
  // has always rendered from rather than changing behaviour under it.
  const scanned = open.every((spec) => withNativeChildren.has(spec.number))
    ? []
    : reads.allIssues();
