// GitHub-backed reads the PRD orchestrator hooks need. The thin `gh`/`git` shell
// around the pure `prd-graph` brain: list issues, list remote branches. Kept here
...
// hooks feed to `prd-graph` to discover a PRD's tracer-bullets and which are done.
...
// detect a live PRD branch (`pickPrdBranch`).
